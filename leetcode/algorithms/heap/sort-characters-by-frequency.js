...
var frequencySort = function(s) {
    ...
    for(let i=0;i<s.length;++i){
        const currentVal = charToFreqMap.get(s[i]);
        charToFreqMap.set(s[i],(currentVal || 0)+1);
    }
    const mapIterator = charToFreqMap[Symbol.iterator]();
    const maxHeap = new Heap((p1,p2) => (p1[1]-p2[1]));
    for(let pair of mapIterator){
        maxHeap.add(pair);
    }
    const pseudoStringBuilder = [];
    while(maxHeap.getSize()>0){
        const currentPair = maxHeap.poll();
        pseudoStringBuilder.push(currentPair[0].repeat(currentPair[1]));
    }
    return pseudoStringBuilder.join('');
};